...
Im2 = imrotate(Im1, 30, 'bicubic', 'crop');
    ...
    %Im1 = V(:,:,:);
    %newIm1 = zeros(size(Im1,2),size(Im1,1),size(Im1,3));
    %for i=1:size(Im1,3)    
    %    newIm1(:,:,i) = imrotate(V(:,:,i),90, 'bicubic');
    %end
    %Im1 = Im1 * 100;

    %you can apply a guassian filter to smooth image
...
%% Step 3: Option 1 - Find Feature Point (Find the phase symmetry, gradient and vesselness of the image)
%This option uses all three images to retrieve information

%phase symmetry
psIm1 = phasesym3D_miccai08_Orientation(Im1,1,6,25,3,.45,3,2,1,.34);
psIm2= phasesym3D_miccai08_Orientation(Im2,1,6,25,3,.45,3,2,1,.34);

%gradient
[vx1 vy1 vz1] = gradient(Im1);
gm1=sqrt(vx1.^2+vy1.^2+vz1.^2);
gm1 = mat2gray(gm1);

[vx2 vy2 vz2] = gradient(Im2);
gm2=sqrt(vx2.^2+vy2.^2+vz2.^2);
gm2 = mat2gray(gm2);

%vesselness

Vf1 = frangiFilter(Im1 ,linspace(3,6,4),70);

if (max(max(max(Vf1))) ~= 1 )
    Vf1=mat2gray(Vf1);
end

Vf2 = frangiFilter(Im2 ,[1 2 3 4],70);

if (max(max(max(Vf2))) ~= 1 )
    Vf2=mat2gray(Vf2);
end

%find feature points for image 1

fprintf('Start to find feature points in Image 1\n');
S1=zeros(2,size(Vf1,1),size(Vf1,2),size(Vf1,3));
...
S2=zeros(2,size(Vf1,1),size(Vf1,2),size(Vf1,3));
...
S1=zeros(1,size(gm1,1),size(gm1,2),size(gm1,3));
...
S2=zeros(1,size(gm2,1),size(gm2,2),size(gm2,3));
